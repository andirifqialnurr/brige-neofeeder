...
const storage = await mkdtemp(path.join(tmpdir(), 'bridge-integration-'));
await mkdir(path.join(storage, 'framework/views'), { recursive: true });
...
const env = {
  ...
  APP_CONFIG_CACHE: path.join(
    root,
    'backend/storage/framework/cache/integration-config-missing.php',
  ),
  DB_CONNECTION: 'mysql',
  DB_URL: '',
  DB_DATABASE: name,
  ...
  REDIS_URL: '',
  REDIS_HOST: process.env.INTEGRATION_REDIS_HOST || '127.0.0.1',
  REDIS_PORT: process.env.INTEGRATION_REDIS_PORT || '6379',
  REDIS_PASSWORD: '',
  REDIS_DB: '0',
  REDIS_CACHE_DB: '0',
  REDIS_PREFIX: `${name}:`,
...
try {
  ...
  for (let i = 0; i < 50; i++) {
    try {
      if ((await fetch(`http://127.0.0.1:${port}/state`)).ok) {
        ready = true;
        break;
      }
    } catch {}
  ...
  await run('php', [
    ...
    '--do-not-cache-result',
    ...(process.argv.includes('--with-backup') ? [] : ['--exclude-group', 'backup']),
  ]);
} catch (err) {
  console.error(err.message);
  process.exitCode = 1;
  ...
  if (provisioned) {
    try {
      await run('php', ['tests/Support/integration-database.php', 'drop']);
    } catch (err) {
      console.error(err.message);
      process.exitCode = 1;
    }
